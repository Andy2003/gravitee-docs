...
== Import an API
In Gravitee.io APIM Console, you can create an PI from scratch or you can import your APIs.
...
==== XML Validation
For each operation, if a `application/xml` request body exist, then a XSD schema is compute from this body to configure an XML Validation policy. +
REQUEST only +
More info link:/apim/3.x/apim_policies_xml_validation.html[here]

==== JSON Validation
For each operation, if a `application/json` request body exist, then a Json schema is compute from this body to configure an JSON Validation policy. +
REQUEST only +
